  ...
  return (
    <Modal title="Add Question">
      ...
      <div className="row mt-26">
        ...
        <div className="col-6">
          <InputField label="Difficulty" value={question.difficulty} dropdown list={difficulties} onchange={(e) => setQuestions({...question, difficulty: e.target.dataset.selected})} autoWidth />
        </div>
      </div>
      <div className='row mt-26'>
        <div className="col-6">
          <InputField label="Type of question" dropdown list={types} value={question.is_true_false ? 'True or False' : 'MCQ'} onchange={(e) => setQuestions({...question, is_true_false: e.target.dataset.selected === 'True or False' ? true : false})} />
        </div>
        <div className='col-6'>
          <InputField autoWidth withInput label="Chapter" type="number" dropdown list={chapters} setList={setChapters} value={chapter.name} onchange={(e) => {
            ...
            setQuestions({...question, chapter: e.target.dataset.id})
          }} />
        </div>
      </div>
      <div className='row mt-26'>
